// step5-pg.mjs — copy data from Supabase to local PG
...
c.on("ready", () => {
  c.exec(`echo ${b64} | base64 -d > /tmp/copy.cjs && echo Mmzzss060112 | sudo -S NODE_PATH=/root/promptopia-web/node_modules bash -c 'cd /root/promptopia-web && node /tmp/copy.cjs' 2>&1`, (e, s) => {
    let o = "";
    s.on("data", (d) => o += d.toString());
    s.stderr.on("data", (d) => o += d.toString());
    s.on("close", () => {
      console.log(o);
      if (o.includes("COPY_DONE")) {
        console.log("✅ Data copied! Rebuilding...");
...
cd /root/promptopia-web && npx prisma generate 2>&1 | tail -2 && npm run build 2>&1 | tail -3 && pm2 restart promptopia && echo BUILD_DONE
'`, (e2, s2) => {
          let o2 = "";
